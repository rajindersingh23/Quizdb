...
var url = `https://rajinder-quizdb.herokuapp.com/${questionId}/options`;
...
function editQuestion(id, ques, answer){
    ...
    if(newQues != ques || correctAnswer != answer){
        var xmlhttp = new XMLHttpRequest();
        var url = `https://rajinder-quizdb.herokuapp.com/${id}`;
        console.log(newQues)
...
function editOptions(options){
    ...
    if(length == 4){
       ...
       newArr.map(data =>{
        var xmlhttp = new XMLHttpRequest();
        var url = `https://rajinder-quizdb.herokuapp.com/options/${data.optionsId}`;
